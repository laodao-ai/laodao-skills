...
> **本段 + [todolist-recorder/SKILL.md](../todolist-recorder/SKILL.md) 的对应段是这套标准唯一的
...
**跨两池共享文件**（不是 buglist 私有，owner 是 `issues-recorder/scripts/issues.py`；
todolist-recorder 依赖同一份，见其 SKILL.md 对应段）：
...
[todolist-recorder/SKILL.md](../todolist-recorder/SKILL.md) 对应段互为镜像）：

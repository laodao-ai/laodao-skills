...
> 分组即拆分线：若 OQ1 决定拆多个 phased change，按下列 §分组切。全部未勾 = 未实现。

## 1. 阶段二：spec-review 编排器（laodao-skills）
...
## 2. 阶段三：impl-review 编排器（laodao-skills）
...
- [ ] 3.3 新增 **issues sweep 步**：`scan --status OPEN --源 {本change}` → 逐项分诊入批次 → 新批次写 batches.md(PLANNED) → hand-off 引用〔I5/I6〕
- [ ] 3.4 弃用官方 `/code-review` 作为独立 step（保留插件能力供历史镜/置信过滤内部借用）〔P3d〕

## 4. 提交自动化（laodao-skills：checkpoint 脚本源 + step prompt + hook 源）
...
## 5. issues 池与批次管理（laodao-skills toolkit：约定 + 脚本；数据迁移属下游 §9）
...
- [ ] 5.4 新增 `reindex` 命令 → 生成 `issues/INDEX.md`（join item+batches，**禁手改**，标记逾期 PLANNED 批次）〔I2/I12〕
...
## 6. 跨模型 outside voice（laodao-skills + trigger-catalog）
...
## 7. workflow bundle 源改写（laodao-skills `opsx-project-init/assets/`，非消费仓副本）
...
## 9. 下游消费仓采纳（**不在本 change 内**，仅登记；各消费仓 update 后各自做）

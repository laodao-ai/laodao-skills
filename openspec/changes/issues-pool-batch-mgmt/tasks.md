...
## 3. reindex + batches 注册表

- [ ] 3.1 加 `reindex` 命令 → 从各 dated 文件重建 `issues/INDEX.md`（**禁手改**；摊清 open item × 批次 + 标 DONE）〔I2，原 §5.4〕
- [ ] 3.2 reindex **顺带同步批次状态**：拿 item 池当 ground truth——成员全 DONE/FIXED → 批次判/标 DONE；状态与成员不一致 → 标出纠正（不静默信手写状态）。**不做逾期主动催办**〔I2/I12·grill-amendment，原 §5.4〕
...
- [ ] 4.1 `opsx-done` 加 **issues sweep 步**（`scan --status OPEN --源 {本change}` → 分诊入批次 → `batches.md`(PLANNED) → hand-off 引用）；**只诊本 change 新增项**〔I5/I6，原 §3.3〕
...
- [ ] 5.3 recorder 脚本测试更新/新增（新路径、批次列兼容、scan 新维度、triage、reindex 含批次状态同步、batch）

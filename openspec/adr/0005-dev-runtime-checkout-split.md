...
- **运行 checkout**（`~/.skills/laodao-skills`）：只 `git pull` 已完成的 skill 并 `setup` 安装；充当全局 **canonical 解析锚点**（`setup` 在此写 `~/.laodao/` 软链/指针，见 `adr/0003` grill-amendment）；只含**已发布**内容，自己**不** run workflow on 自己。

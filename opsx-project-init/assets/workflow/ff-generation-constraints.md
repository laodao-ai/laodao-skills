...
> 所有调用方（`openspec/workflow-serverside.md` 的 ff prompt、对比表、判定速查）
...
D-1~D-5 原本以纯文本形式内嵌在 `workflow-serverside.md` 的 `/opsx:ff` 命令 prompt 单元格里
...
D 约束是稀缺资源——塞太多会触发过载（见 `workflow-serverside.md` §"反面教训"）。
...
`workflow-serverside.md` 等调用方**不得复制上表定义文本**，只能写形如：
...
```
# workflow-serverside.md 在多个调用点各写一份完整 D 定义，
# 对比表/附录/速查又各写一遍裸编号清单
...
- [ ] `workflow-serverside.md` 各处是否均为编号引用，无定义复制？
...
- 2026-06-28：从 `workflow-serverside.md` 内嵌文本抽出，建立单一权威源。修正历史上 D-5 未同步到对比表/附录/速查导致的定义漂移。

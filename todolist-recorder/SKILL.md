...
> **本段 + [buglist-recorder/SKILL.md](../buglist-recorder/SKILL.md) 的对应段是这套标准唯一的
...
**跨两池共享文件**（不是 todolist 私有，owner 是 `issues-recorder/scripts/issues.py`；
buglist-recorder 依赖同一份，见其 SKILL.md 对应段）：
...
（防"空集全称为真"的 vacuous-truth 假 DONE）。**注意**：bug 池终态集是 `{FIXED, WONTFIX}`——
...
**共享**（`issues-recorder/scripts/issues.py`，独占跨 bug+todo 的命令；与
[buglist-recorder/SKILL.md](../buglist-recorder/SKILL.md) 对应段互为镜像）：
...
`opsx-done` 生成 hand-off 那一步跑 sweep：以 **源==本change ∧ status 非终态 ∧ 批次==空**
